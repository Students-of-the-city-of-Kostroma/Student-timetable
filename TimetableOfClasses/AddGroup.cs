...
namespace TimetableOfClasses
{
	public partial class AddGroup : Form
	{
						...
						if (ushort.TryParse(nudMinPar.Value.ToString(), out minPar))
							if (ushort.TryParse(nudMaxPar.Value.ToString(), out maxPar))
								if (nudMaxPar.Value >= nudMinPar.Value)
								{
									...
									if (LibOfTimetableOfClasses.RefData.CGroup.Insert(Group))
										return true;
								...
								else
								{
									...
									if (LibOfTimetableOfClasses.RefData.CGroup.Update(group))
										return true;
		...
		{
			if (LibOfTimetableOfClasses.RefData.CTrainingProfile.Rows.Count == 0)
			{
