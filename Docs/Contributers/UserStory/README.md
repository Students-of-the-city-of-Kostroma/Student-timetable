...
Пользовательсякая история представляет из себя текстовый файл с именем `Story-<ПорядковыйНомер>.md`, например `Story-10.md`. Файлы располанается в дирректории `/Docs/Technical/UserStories/`, пример относительного пути к файлу `/Docs/Technical/UserStories/Story-10.md`
