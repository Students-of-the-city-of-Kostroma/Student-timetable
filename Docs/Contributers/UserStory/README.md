...
Пользовательсякая история представляет из себя текстовый файл с именем `READMY.md`. Файлы располагается в дирректории `/Docs/Technical/UserStories/Story-100`, пример относительного пути к файлу `/Docs/Technical/UserStories/Story-100/READMY.md`, где 100 это порядковый номер пользовательской истории.
...
- После всех шагов мы описываем "Ожидаемый результат", который и будет являтся проверкой тестов;
- Количество тест-кейсов не менее трех.

Сценарий 1: Агрессивный ордер в регулярной торговле торгуется не вставая на книжку.

Послать пассивный ордер с ценой продажи 100 и объемом 1000
Послать агрессивный ордер с ценой покупки 100 и объемом 1000
Ожидаемый результат: Пользователь получает два сообщения о торговле на весь объем ордеров. Сообщения о том, что агрессивный ордер попал на книжку не приходит.
Относительно интерфейса, нужно описывать что пользователь должен вводить, что бы достичь какого-т результата.
### Технические заметки.
...
**ВАЖНО:** После тестирования мы должны ответить на вопрос: `Соответствует ли User Story критериям приемки?`
