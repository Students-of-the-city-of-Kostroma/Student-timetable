...
Должна отражать суть задачи и быть легко узнаваемой среди других задач

### Поле Ответственные (Assignees)
В этом поле указывается человек (или общая учетная запись команды), который в данный момент должен заниматься этой задачей. Задача может назначаться другому человеку, при этом с себя задачу нужно снять, так как Вы ее передаете для решения какого-либо вопроса. Задача не должна находиться без исполнителя. Сначала необходимо добавить все изменения в задачу, а когда последние изменения будут сделаны, передать задачу. Случаи, когда задача может передаться другому пользователю:
- Для проверки выполнения этой задачи
- Для уточнение информации по этой задаче
- В случае если Вы не можете выполнять задачу (увольнение, длительная болезнь и т.д.)
...
# Labels:
- Story - Задача на разработку функциональности. Все задачи относятся к какой-то сущности, например: Преподаватель, Группа, Аудитория, Дисциплина и т.д. Задача должна иметь соответствующую этой сущности одноименную англоязычную метку.
-  new() - Если задача имеет отношение к конструктору, то указывается метка.
- Insert, Update, Delete - Если задача относится к методу Insert, Update, Delete, то указывается соответственно метка Insert(Model model), Update(Model model), Delete (Model model).
