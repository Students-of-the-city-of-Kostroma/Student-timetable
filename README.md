...
- Git bush
# Жизненный цикл issue (задачи):
Задача - это активность за выполнение которой начисляютс баллы. 1 issue = 1 балл. Каждая задача может иметь любое количество меток, они определяют тип задачи, этап ее выполнения и принадлежность к функционалу разрабатываемого ПО. Если задача требует изменение исходного кода проекта, то для этой задачи создается отдельная ветка с именем соответвтвующим номеру задачи по щаблону issue-***, например issue-123. Для таких задач требуются запрос на слияние. Нельзя приступать к выполнению задачи, если связанные с нейзадачи не выполнены.
## Задачи с меткой Story:
...
- Студент реализует задачу в ветку и запрашывает запрос на слияние. При это имя запроса должно соответсвовать шаблону "WIP: ИмяЗадачиДляКоторойВыполняетсяЗапрос",Ю  а описание должно содержать текст "Закрывает #123"
...
- Задача имеет исполнителя и помечена меткой To do - это значит что эту задачу планирует исполнять. Если по задаче нет активности в течении недели, то задача будет снята с исполнителя и метка To do убрана. Не больше двух задач на одного исполнителя в этом состоянии.
