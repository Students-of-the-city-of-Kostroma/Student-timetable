...
# Labels:
- Story - Задача на разработку функциональности. Все задачи относятся к одной из четырех сущностей: Преподаватель, Группа, Аудитория, Дисциплина. Задача должна иметь соответствующую этой сущности одноименную англоязычную метку. Если упомянут класс, который начинается с большой буквы M, например, MGroup, то задача должна быть помечена меткой Model. Если задача имеет отношение к конструктору, методу Insert, Update или Delete , то задача должна быть помечена меткой Controller. Если задача имеет отношение к конструктору, то указывается метка new(). Если задача относится к методу Insert, Update, Delete, то указывается соответственно метка Insert(Model model), Update(Model model), Delete (Model model).
