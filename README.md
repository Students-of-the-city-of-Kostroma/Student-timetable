...
- Студент реализует задачу в ветку и запрашивает запрос на слияние. При это имя запроса должно соответсвовать шаблону "WIP: ИмяЗадачиДляКоторойВыполняетсяЗапрос",  а описание должно содержать текст "Закрывает #123"
...
- Ни на кого неназначеные открытые задачи без меток состояния выплнения - эти задачи формируют список задач из которого участник проекта может взять на исполнение.
...
- Задача имеет исполнителя и помечена меткой Doing - это значит, что к исполнению этой задачи уже приступили. Если нет активности в течении недели, то у задачи будет снята метка Doing и выставлена метка To do. Не более одной задачи на одного исполнителя в этом состоянии.
